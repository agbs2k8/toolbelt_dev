...
class CountVectorizer(BaseEstimator):
    ...
    Examples
    --------
    >>> from toolbelt.feature_extraction.rules import CountVectorizer
    >>> host_log_list = [
    ...
    def __init__(self, max_hf=1.0, min_hf=1, max_features=None, logs=None, dtype=np.int64):
        self.max_hf = max_hf
        self.min_hf = min_hf
    ...
    def transform(self, log_lists):
        ...
        _, X = self._count_logs(log_lists, fixed_rules=True)
        return X
...
class LfihfTransformer(BaseEstimator, TransformerMixin):
    """Transform a count matrix to a normalized hf or lf-ihf representation
        ...
        sublinear_lf : boolean (default=False)
            Apply sublinear lf scaling, i.e. replace lf with 1 + log(lf).
    ...
    def __init__(self, norm='l2', use_ihf=True, smooth_ihf=True,
        ...
        self.sublinear_lf = sublinear_lf
    ...
    def transform(self, X, copy=True):
        ...
        if self.sublinear_lf:
            np.log(X.data, X.data)
            X.data += 1

        # TODO: 1. Update the LF portion to allow for Boolean frequencies, f(l,h) = 1 if l>0, else 0
        #       2. LF adjusted for host-log-set length lf(l,h) = f(l,h) / len(h)
        #       3. Log Scaled: lf(l,h) = log(1+ f(l,h))
        #       4. Augmented Frequency = https://en.wikipedia.org/wiki/Tf%E2%80%93idf
...
class LfihfVectorizer(CountVectorizer):
    """Convert a collection of log IDs to a matrix of Log Freqency - Inverse Host Frequency (LF-IHF) features.
    ...
    def __init__(self, max_hf=1.0, min_hf=1, max_features=None, logs=None, dtype=np.float64, norm='l2', use_ihf=True,
                 smooth_ihf=True, sublinear_lf=False):
        super().__init__(max_hf=max_hf, min_hf=min_hf, max_features=max_features, logs=logs, dtype=dtype)

        self._lfihf = LfihfTransformer(norm=norm, use_ihf=use_ihf, smooth_ihf=smooth_ihf, sublinear_lf=sublinear_lf)
    ...
    @property
    def sublinear_lf(self):
        return self._lfihf.sublinear_lf

    @sublinear_lf.setter
    def sublinear_lf(self, value):
        self._lfihf.sublinear_lf = value
