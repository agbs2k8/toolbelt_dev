...
def import_reference_file():
    stock_reference = pd.read_csv(os.path.join(__location__, 'stock_reference.txt'), sep='\t', encoding='latin-1')
...
def get_key_ratios(ticker):
    ...
    key_ratios['ev_bv'] = stock_reference['Total Enterprise Value'] / (
    stock_reference['Total Assets'] - stock_reference['Total Liabilities'])
    key_ratios['bv_share'] = (stock_reference['Total Assets'] - stock_reference['Total Liabilities']) / stock_reference[
        'Shares Outstanding']
    ...
    # graham
    key_ratios['graham_value'] = (stock_reference['Diluted EPS Incl Extra Items'] * (
    8.5 + (2 * (stock_reference['LT EPS Growth Rate']))) * 4.4) / (
...
def get_current(ticker):
    ...
    url = "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=" + ticker + "&interval=1min&apikey=" + \
          credentials['api_key']
